...
class Evaluator {
    ...
    public function evaluate( Ast\Body $source, $state ) {
        return trim( $this->evaluateBody( $source, new Context( $this, null, new State( $state ) ),
            new Chunk( $this ) )->getOut() );
    }
        ...
        elseif ( $section->type == '@' ) {
            ...
            switch ( $section->type ) {
                case '#':
                    ...
                    elseif ( is_iterable( $resolved ) ) {
                        //array means loop
                        $iterationCount = - 1;
                        foreach ( $resolved as $index => $value ) {
                            //run body
                            $chunk = $this->evaluateBody( $section->body,
                                $ctx->push( $value, $index, count( $resolved ), ++ $iterationCount ),
                                $chunk );
                        }
                    }
    ...
    public function evaluateElseBody( Ast\Section $section, Context $ctx, Chunk $chunk ) {
        if ( $section->bodies != null && ( is_countable( $section->bodies ) ? count( $section->bodies )
                : 0 ) > 0 ) {
    ...
    public function evaluateReference( Ast\Reference $ref, Context $ctx, Chunk $chunk ) {
        ...
        if ( ! $this->isEmpty( $resolved ) ) {
            ...
            if ( empty( $ref->filters ) ) {
                //default filters
                $resolved = array_reduce( $this->dust->automaticFilters,
                    fn( $prev, Filter\Filter $filter ) => $filter->apply( $prev ), $resolved );
            }
            else {
