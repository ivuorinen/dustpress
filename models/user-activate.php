...
class UserActivate extends \DustPress\Model {
    ...
    public function State() {
        ...
        if ( is_wp_error( $result ) && in_array( $result->get_error_code(), $valid_error_codes, true ) ) {
            ...
            $this->signup         = $signup;
            $this->print['title'] = apply_filters(
                'dustpress/activate/account_active',
                __( 'Your account is now active!' )
            );

            if ( $signup->domain . $signup->path === '' ) {
                // account active and email sent
                return $this->StateAccountActiveEmailSent();
            }

            // site active and email sent
            return $this->StateSiteActiveEmailSent();
        }
        if ( $result === null || is_wp_error( $result ) ) {
            $state                = 'error';
            $this->print['title'] = apply_filters(
                'dustpress/activate/error_occurred',
    ...
    /**
     * Returns state when No key has been set
     *
     * @return string
     */
    public function StateNoKey() {
        $state                           = 'no-key';
        $this->print['title']            = apply_filters(
            'dustpress/activate/key_required',
            __( 'Activation Key Required' )
        );
        $this->print['wp-activate-link'] = network_site_url( 'wp-activate.php' );
        $this->state                     = $state; // No idea why this would be needed

        return $state;
    }

    /**
     * Returns state where account has been activated and email sent
     *
     * @return string
     */
    public function StateAccountActiveEmailSent() {
        ...
        $this->print['message'] = apply_filters(
            ...
            sprintf( /* translators: 1: login URL, 2: username, 3: user email, 4: lost password URL */
                ...
                network_site_url( 'wp-login.php', 'login' ),
                $this->signup->user_login,
                $this->signup->user_email,
                wp_lostpassword_url()
            ),
            $this->signup->user_login,
            $this->signup->user_email
    ...
    /**
     * Returns state where site has been activated and email sent
     *
     * @return string
     */
    public function StateSiteActiveEmailSent() {
